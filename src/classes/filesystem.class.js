    ...
    constructor(opts) {
            ...
            await new Promise((resolve, reject) => {
                ...
                content.forEach(async (file, i) => {
                    ...
                    if (typeof fstat !== "undefined") {
                        ...
                        if (e.category === "dir" && tcwd === settingsDir && file === "themes") e.type="voidcore-themesDir";
                        if (e.category === "dir" && tcwd === settingsDir && file === "keyboards") e.type = "voidcore-kblayoutsDir";
                    ...
                    if (e.category === "file" && tcwd === themesDir && file.endsWith(".json")) e.type = "voidcore-theme";
                    if (e.category === "file" && tcwd === keyboardsDir && file.endsWith(".json")) e.type = "voidcore-kblayout";
                    if (e.category === "file" && tcwd === settingsDir && file === "settings.json") e.type = "voidcore-settings";
                    if (e.category === "file" && tcwd === settingsDir && file === "shortcuts.json") e.type = "voidcore-shortcuts";
        ...
        this.render = async (originBlockList, isDiskView) => {
            ...
            blockList.forEach((e, blockIndex) => {
                ...
                if (e.type === "voidcore-theme") {
                    cmd = `window.themeChanger("${e.name.slice(0, -5)}")`;
                }
                if (e.type === "voidcore-kblayout") {
                    cmd = `window.remakeKeyboard("${e.name.slice(0, -5)}")`;
                }
                if (e.type === "voidcore-settings") {
                    cmd = `window.openSettings()`;
                }
                if (e.type === "voidcore-shortcuts") {
                ...
                switch(e.type) {
                    ...
                    case "usb":
                        ...
                        break;
                    case "voidcore-theme":
                        icon = this.voidcoreIcons.theme;
                        type = "VoidCore UI theme";
                        break;
                    case "voidcore-kblayout":
                        icon = this.voidcoreIcons.kblayout;
                        type = "VoidCore UI keyboard layout";
                        break;
                    case "voidcore-settings":
                    case "voidcore-shortcuts":
                        icon = this.voidcoreIcons.settings;
                        type = "VoidCore UI config file";
                        break;
                    case "system":
                        icon = this.voidcoreIcons.settings;
                        break;
                    case "voidcore-themesDir":
                        icon = this.voidcoreIcons.themesDir;
                        type = "VoidCore UI themes folder";
                        break;
                    case "voidcore-kblayoutsDir":
                        icon = this.voidcoreIcons.kblayoutsDir;
                        type = "VoidCore UI keyboards folder";
                        break;
                    default:
