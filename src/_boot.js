...
process.on("uncaughtException", e => {
    signale.fatal(e);
    dialog.showErrorBox("VoidCore UI crashed", e.message || "Cannot retrieve error message.");
    if (tty) {
        tty.close();
    }
    if (extraTtys) {
...
signale.start(`Starting VoidCore UI v${app.getVersion()}`);
signale.info(`With Node ${process.versions.node} and Electron ${process.versions.electron}`);
signale.info(`Renderer is Chrome ${process.versions.chrome}`);

const gotLock = app.requestSingleInstanceLock();
if (!gotLock) {
    signale.fatal("Error: Another instance of VoidCore is already running. Cannot proceed.");
    app.exit(1);
}
